...
func _ready() -> void:
    ...
    SignalManager.addTextBox.connect(add_text_box)
...
func set_focused_interactable(node: Node) -> void:
    if node.get_parent() != null:
        var parent_node = node.get_parent()
        if parent_node is Interactable:
            focused_interactable = parent_node
            print_debug('Setting GameManager.focused_interactable to %s.' % focused_interactable)
            return
    if node.get_parent() == null:
        focused_interactable = null
        push_error('Could not set %s as focused interactable. Its camera position could not find it.' % node.name)
        return
    return
