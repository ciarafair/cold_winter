...
func _process(_delta: float) -> void:
    ...
    game_manager.camera_position_dictionary.main_camera_position = self
...
func find_game_manager():
    ...
    if parent == null:
        push_error('Could not find the parent node for MainCameraPosition.')
        return null
    var node = parent.get_parent()
    if node == null:
        push_error('Could not find the GameManager node for %s.' % parent.name)
        return null
    #print_debug('Setting game_manager for MainCameraPosition to %s.' % node.name)
    return node
